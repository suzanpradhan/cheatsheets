...
import {Card, Badge, Group, Button, TextInput, Textarea, Avatar} from '@mantine/core';
import {IconCopy, IconTrash, IconPencilMinus, IconChecks} from '@tabler/icons-react';
            ...
            <div className="flex items-center justify-between">
                ...
                {isHovered && (
                    <Group gap="xs">
                        ...
                        {/*    onClick={handleEdit}*/}
                        {/*>*/}
                            <IconPencilMinus size={14} className={"text-slate-300 hover:text-black transition-colors cursor-pointer"} onClick={handleEdit}  />
                        {/*</ActionIcon>*/}
                        {/*<ActionIcon*/}
                        {/*    variant="transparent"*/}
                        {/*    size="sm"*/}
                        ...
                        {/*>*/}
                            <IconTrash size={14} className={"text-slate-300 hover:text-red-600 transition-colors cursor-pointer"}  onClick={onDelete} />
                        {/*</ActionIcon>*/}
                    </Group>
                )}
            </div>
            ...
            <Group justify="space-between" className="text-xs text-slate-400 mt-4">
                <Group gap="xs">
                    <Avatar radius={"sm"} size={20} >
                        {snippet.author.split(' ').map(word => word[0]).join('').toUpperCase().slice(0, 2)}
                    </Avatar>
                    <div className="text-slate-500 text-xs">{snippet.date}</div>
                </Group>

                <div className={`flex items-center gap-2 text-xs transition-colors cursor-pointer ${copied ? 'text-black' : 'text-slate-400 hover:text-black'}`} onClick={handleCopy}>
